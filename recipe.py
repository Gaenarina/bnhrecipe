...
def generate_recipe():
    # Extract three ingredients from the user's input
    ...
    # Create a compact and keep the output simple
    prompt = f"Craft a recipe in HTML in {selected_language} using \
        ...
        Use only basic HTML tags such as h2, h3, p, ul, li, ol.\
        Do not use style, css, div, class, markdown, or code block.\
        Do not include English translations or parenthetical translations.\
    ...
    try:
        ...
        recipe = recipe.replace("```html", "").replace("```", "")
